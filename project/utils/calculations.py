...
def calculate_availability(df, scheduled_time):
    ...
    # Filter out PCP stops
    pcp_stops = df[df['Área Responsável'] == 'PCP']['Duração'].sum()
    total_downtime = df['Duração'].sum() - pcp_stops
...
def calculate_operational_efficiency(df, scheduled_time):
    ...
    # Filter out Operation and Organizational stops
    excluded_areas = ['Operação', 'Organizacional']
    excluded_stops = df[df['Área Responsável'].isin(excluded_areas)]['Duração'].sum()
    operation_time = scheduled_time - excluded_stops
    efficiency = operation_time / scheduled_time * 100
...
def generate_recommendations(df, availability, efficiency):
    ...
    # Check efficiency
    if efficiency < 65:
        recommendations.append("⚠️ A eficiência operacional está baixa. Analise as causas mais frequentes de paradas e implemente ações corretivas.")
    elif efficiency < 80:
        recommendations.append("⚠️ A eficiência operacional está em um nível moderado. Busque otimizar processos para reduzir o tempo de parada.")
    else:
        recommendations.append("✅ A eficiência operacional está em um bom nível. Continue com as práticas atuais de manutenção.")
