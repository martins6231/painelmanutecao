...
def create_area_pie_chart(area_index, language='pt'):
    ...
    fig.update_traces(
        ...
        pull=[0.05 if i == area_index.values.argmax() else 0 for i in range(len(area_index))]  # Highlight the largest value
    )
...
def create_comparison_gauge_chart(value1, value2, title, max_value=100, language='pt'):
    ...
    # Determine the color based on whether an increase is good or bad
    # For metrics like availability and efficiency, an increase is good
    # For metrics like downtime and MTTR, a decrease is good
    if title.lower() in ['availability', 'efficiency', 'mtbf', 'disponibilidade', 'eficiência']:
        color = "#2ecc71" if diff > 0 else "#e74c3c"  # Green for increase, red for decrease
    else:  # For metrics like MTTR, downtime, etc.
        color = "#2ecc71" if diff < 0 else "#e74c3c"  # Green for decrease, red for increase
